...
import { likeBlog, deleteBlog, deleteOne } from "../reducers/blogReducer";
...
const Blog = ({blog, user, setUser}) => {
  ...
  const like = (blog) => {
    ...
    dispatch(likeBlog(voted))
    dispatch(showNotification(`you voted ${voted.title}`, 5))
  }

  const deleteOne = (blog) => {
    const deletedID = blog._id
    dispatch(deleteBlog(deletedID))
    dispatch(showNotification(`you deleted ${blog.title}`, 5))
  }
    ...
    <div style={blogStyle} className="blog">
          <div >
            ...
            { user && user.username === blog.user.username ?
              <button onClick={()=>deleteOne(blog)}>Delete</button> :
              null
            }
          </div>
    </div>
